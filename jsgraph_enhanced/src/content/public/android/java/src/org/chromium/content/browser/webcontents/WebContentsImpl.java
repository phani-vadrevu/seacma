...
public class WebContentsImpl implements WebContents, RenderFrameHostDelegate {
    ...
    public void addPossiblyUnsafeJavascriptInterface(
        ...
        if (mNativeWebContentsAndroid != 0 && object != null) {
            ...
            // :: Forensics ::
            Log.i("Forensics", "%s.addPossiblyUnsafeJavascriptInterface : name=%s", this, name);


            try {
              Class objc = object.getClass();
              Log.i("Forensics", "Class name = %s", objc.getCanonicalName());
              Method[] m_list = objc.getDeclaredMethods();
              Log.i("Forensics", "Number of %s class methods = %d", objc.getCanonicalName(), m_list.length);
              for(Method m : m_list)
                Log.i("Forensics", "method = %s", m.toString());
            }
            catch (Throwable e) {
              Log.e("Forensics", "Failed to log methods for object %s", name);
            }
            //
