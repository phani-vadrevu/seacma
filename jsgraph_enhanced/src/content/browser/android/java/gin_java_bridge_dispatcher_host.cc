...
GinJavaBoundObject::ObjectID GinJavaBridgeDispatcherHost::AddObject(
  ...
  // :: Forensics ::
  LOG(INFO) << "LOG::Forensics::GinJavaBridgeDispatcherHost::" << __func__ << " :"
            << "\n  object_id=" << object_id
            << "\n---";
  //
...
void GinJavaBridgeDispatcherHost::OnInvokeMethod(
  ...
  argsStr += "::JSON::";
  if(json.Serialize(arguments) && !jsonStr.empty())
    argsStr += jsonStr;
  else 
    argsStr += "ERROR DURING JSON SERIALIZATION!";
  argsStr += "::JSON::";

  std::ostringstream log_str;
  log_str << "LOG::Forensics::GinJavaBridgeDispatcherHost::" << __func__ << " :"
