...
void Document::MaybeHandleHttpRefresh(const String& content,
  ...
  // :: Forensics ::
  Forensics::DidHandleHttpRefresh(frame_, refresh_url, http_refresh_type);  
  //
}
...
int Document::RequestAnimationFrame(
  ...
  int id = EnsureScriptedAnimationController().RegisterCallback(callback);

  // :: Forensics ::
  Forensics::RequestAnimationFrame(frame_, id);  
